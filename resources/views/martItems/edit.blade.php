        ...
        <div class="container-fluid">
            <div class="card border">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('mart-items.update', $item->id) }}"
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="vendorID" id="vendorID" required>
                                ...
                                @foreach($vendors as $vendor)
                                    <option
                                        value="{{ $vendor->id }}" {{ old('vendorID', $item->vendorID) == $vendor->id ? 'selected' : '' }}>
                                        {{ $vendor->title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="number" step="0.01" min="0" class="form-control" id="disPrice"
                                       name="disPrice" value="{{ old('disPrice', $item->disPrice) }}">
                            </div>
                        </div>
                                ...
                                <select class="form-control" id="categoryID" name="categoryID" required>
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}"
                                                data-section="{{ $category->section }}" {{ old('categoryID', $item->categoryID) == $category->id ? 'selected' : '' }}>
                                            {{ $category->title }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control" id="brandID" name="brandID">
                                    ...
                                    @foreach($brands as $brand)
                                        <option
                                            value="{{ $brand->id }}" {{ old('brandID', $item->brandID) == $brand->id ? 'selected' : '' }}>
                                            {{ $brand->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" id="section" name="section"
                                       value="{{ old('section', $item->section) }}" readonly>
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                @if ($item->photo)
                                    <div class="mt-2">
                                        <img src="{{ $item->photo }}" alt="item image" style="height:80px"
                                             class="rounded">
                                        <div class="form-check mt-2">
                                            <input type="checkbox" class="form-check-input" id="remove_photo"
                                                   name="remove_photo" value="1">
                                            <label class="form-check-label" for="remove_photo">Remove current
                                                image</label>
                                        </div>
                                    </div>
                                @endif
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <input type="number" class="form-control" id="grams" name="grams" min="0"
                                       value="{{ old('grams', $item->grams) }}">
                            </div>
                            <div class="form-group col-md-3">
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <input type="number" class="form-control" id="fats" name="fats" min="0"
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" id="{{ $field }}"
                                                   name="{{ $field }}"
                                                   value="1" {{ old($field, $item->$field) ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            <div class="form-check form-check-inline">
                                <input type="checkbox" class="form-check-input" id="publish" name="publish"
                                       value="1" {{ old('publish', $item->publish) ? 'checked' : '' }}>
                                <label class="form-check-label" for="publish">Publish</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input type="checkbox" class="form-check-input" id="isAvailable" name="isAvailable"
                                       value="1" {{ old('isAvailable', $item->isAvailable) ? 'checked' : '' }}>
                                <label class="form-check-label" for="isAvailable">Available</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input type="checkbox" class="form-check-input" id="nonveg" name="nonveg"
                                       value="1" {{ old('nonveg', $item->nonveg) ? 'checked' : '' }}>
                        ...
                        <fieldset>
                            ...
                            <div class="form-check width-100">
                                <input type="checkbox" class="has_options" id="has_options" name="has_options" value="1" {{ old('has_options', $item->has_options) ? 'checked' : '' }}>
...
@section('scripts')
    <script>
        ...
        $(document).ready(function() {
            ...
            @if($item->has_options && $item->options)
                @php
                    $existingOptions = is_string($item->options) ? json_decode($item->options, true) : $item->options;
                    $existingOptions = is_array($existingOptions) ? $existingOptions : [];
                @endphp
                @if(!empty($existingOptions))
                    var existingOptions = @json($existingOptions);
                    if (existingOptions && existingOptions.length > 0) {
                        // Enable options checkbox and show sections
                        hasOptionsCheckbox.prop('checked', true);
                        toggleOptionsVisibility(false);
                        
                        // Load each option
                        existingOptions.forEach(function(optionData, index) {
                            loadExistingOption(optionData, index + 1);
                        });
                        
                        // Update summary and default option after a short delay to ensure DOM is updated
                        setTimeout(function() {
                            updateOptionsSummary();
                            updateDefaultOptionSelect();
                            
                            // Set default option if exists
                            @if($item->default_option_id)
                                $('#default_option').val('{{ $item->default_option_id }}');
                            @endif
                        }, 100);
                    }
                @endif
        ...
        // Load existing option from saved data
        function loadExistingOption(optionData, optionNumber) {
            optionCounter++;
            const optionId = optionData.id || 'option_' + Date.now() + '_' + optionCounter;
            ...
            template.find('.option-title').val(optionData.option_title || $('#name').val() || '');
            template.find('.option-subtitle').val(optionData.option_subtitle || '');
            template.find('.option-unit-price').val(optionData.unit_price || 0);
            template.find('.option-original-unit-price').val(optionData.original_unit_price || optionData.unit_price || 0);
            ...
            if (optionData.image) {
                template.find('.option-image-preview').html(
                    `<img src="${optionData.image}" style="max-width: 100px; max-height: 100px; border-radius: 4px; margin-top: 10px;" class="img-thumbnail">`
                );
            }
            ...
            optionsList.push({
                ...
                title: optionData.option_title || $('#name').val() || '',
                ...
                total_price: parseFloat(optionData.price) || parseFloat(optionData.total_price) || 0,
                original_total_price: parseFloat(optionData.original_price) || parseFloat(optionData.original_total_price) || 0,
            ...
            updateSmartSuggestions(optionId, optionData.option_type || 'size');
            
            // Apply visual states
            if (optionData.is_available !== false) {
                template.addClass('option-enabled');
            } else {
                template.addClass('option-disabled');
            }
            
            if (optionData.is_featured === true) {
                template.addClass('option-featured-highlight');
            }
        }
