            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        // Date should be formatted from server, but handle both cases
                        if (!data || data === '-' || data === '') {
                            return '-';
                        }
                        
                        // If we get ISO format (like "2025-07-23T18:30:00.306000Z"), format it client-side
                        if (typeof data === 'string' && data.includes('T')) {
                            try {
                                // Remove quotes if present (handle JSON-encoded strings)
                                var dateStr = data.replace(/^["']|["']$/g, '');
                                
                                // Parse the ISO date string
                                var date = new Date(dateStr);
                                
                                if (!isNaN(date.getTime())) {
                                    // Convert to Asia/Kolkata timezone using Intl.DateTimeFormat
                                    ...
                                    return weekday + ' ' + month + ' ' + day + ' ' + year + ' ' + hour + ':' + minute + ':' + second + ' ' + dayPeriod.toUpperCase();
                                }
                            } catch(e) {
                                console.error('Date formatting error:', e, 'Data:', data);
                            }
                        }
